...
### Сохранность регулярных операций

Новая регулярная операция помечается как ожидающая подтверждения до отправки.
При слиянии сохраняются ожидающие записи и локальные записи, которых ещё не было
в подтверждённом облачном списке, даже если с момента их создания уже прошла
другая синхронизация. Такие записи остаются доступными для повторной отправки.
Подтверждение снимается только по совпадающей записи из ответа сервера, а не
по локальному состоянию после попытки загрузки. Явные локальные удаления и
удаления ранее синхронизированных записей без ожидающих изменений сохраняются.

Проверка: `node --import tsx --test tests/recurring-create-sync.test.ts`.
