...
function buildFinancialContextLines(context?: AdvisorFinancialContext) {
  ...
  const actualSpendingLines =
    (context.expenses.spending ?? []).length > 0
      ? (context.expenses.spending ?? [])
          .slice(0, 5)
          .map(
            (spending) =>
              `- ${spending.category}: ${spending.amount} RUB за текущий период (${spending.transactions} операций)`,
          )
          .join("\n")
      : "- Нет подтверждённых расходов за текущий период";
  ...
  return [
    ...
    "Лимиты:",
    budgetLines,
    "Фактические расходы текущего периода по категориям:",
    actualSpendingLines,
    "Цели:",
    goalLines,
...
function buildFinancialBriefLines(brief?: FinancialAdviserBrief | null) {
  ...
  const goalPlanBlock = brief.goalPlan
    ? [
        "Готовый маршрут к цели:",
        `- Цель: ${brief.goalPlan.targetAmount} RUB`,
        `- Уже отложено: ${brief.goalPlan.savedAmount} RUB`,
        `- Осталось накопить: ${brief.goalPlan.remainingAmount} RUB`,
        `- Безопасный ежемесячный темп по текущему плану: ${brief.goalPlan.monthlyContribution} RUB`,
        brief.goalPlan.monthsAtCurrentContribution
          ? `- Срок в текущем темпе: ${brief.goalPlan.monthsAtCurrentContribution} месяцев`
          : "- В текущем плане нет положительного ежемесячного темпа для цели",
        "Варианты темпа:",
        ...(brief.goalPlan.scenarios.length > 0
          ? brief.goalPlan.scenarios.map(
              (scenario) =>
                `- ${scenario.label}: ${scenario.monthlyContribution} RUB в месяц, ${scenario.months} месяцев`,
            )
          : ["- Варианты темпа появятся после высвобождения ежемесячного взноса"]),
      ].join("\n")
    : null;
...
export function getAdvisorSystemPrompt(args: {
  ...
  if (args.locale === "ru") {
    return [
      ...
      "Если вопрос про расходы, месяц или экономию, сначала разбери текущий период: назови 1–3 самые крупные фактические категории расходов, их суммы и только затем предложи конкретное сокращение. Не называй обязательный платёж кандидатом на сокращение без прямого запроса клиента.",
      "Для вопроса о текущем месяце сначала отвечай про оставшиеся дни текущего периода. Риск в следующем месяце можно упомянуть одной короткой строкой в конце только как следующий шаг, а не как главный вывод.",
      "Если фактических расходов по категориям нет, честно скажи это и попроси записывать траты с категорией вместо общих фраз о сокращении расходов.",
      "Если вопрос про крупную покупку, сначала покажи разрыв и влияние на план, а потом предложи план покупки.",
      "Если вопрос про накопление, дом или другую цель, используй «Готовый маршрут к цели» как основу ответа. Не заменяй его общими фразами про вклады, инвестиции или дополнительный доход.",
      "Для вопроса о цели дай личный план в четырёх коротких блоках: 1) реальность сейчас — цель, уже отложено, остаток и текущий темп; 2) варианты — только готовые варианты темпа и срока из контекста; 3) действие на ближайшие 7 дней — один конкретный шаг в FIN OS; 4) одно уточнение, которое сильнее всего улучшит план. Если текущий темп равен нулю, прямо скажи, что сначала нужно высвободить взнос.",
      "Не придумывай доходность вкладов, инвестиционные инструменты, кредиты, ипотеку, банки или сторонние контакты. Не предлагай обращаться в Telegram или к внешнему консультанту.",
      ...
      "Пиши только обычным текстом. Никогда не используй Markdown, символы *, #, _, обратные кавычки, таблицы или ссылки. Заголовки пиши простой строкой с двоеточием.",
